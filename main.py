...
class gui:
    ...
    face = []
    sollution = []
    solution_history = []  # NEW: Store history of moves for back functionality
    green_str = "FFFFFFFFF"
    white_str = "UUUUUUUUU"
    red_str = "RRRRRRRRR"
    ...
    def get_face_rep_with_arrow(self, face_stat, clockwise=True, Double=False):
        ...
        if not clockwise:
            img = cv2.arrowedLine(img,(190,17),(10,17),(255,255,255),4)
            if Double:
                img = cv2.arrowedLine(img,(20,190),(200,190),(255,255,255),4)
        elif clockwise:
            img = cv2.arrowedLine(img,(20,17),(200,17),(255,255,255),3)
            if Double:
                img = cv2.arrowedLine(img,(190,190),(10,190),(255,255,255),3)  
    ...
    def scan_green(self):
        if self.frozen_detection and len(self.frozen_detection) == 9:
            ...
            if len(side) == 9 and side[4] == 0:  # FIXED: Check length first
                ...
                self.panel0 = Label(self.root, image=img0, bg="#2b2b2b")
    ...
    def scan_white(self):
        if self.frozen_detection and len(self.frozen_detection) == 9:
            ...
            if len(side) == 9 and side[4] == 1:  # FIXED: Check length first
                ...
                self.panel1 = Label(self.root, image=img1, bg="#2b2b2b")
    ...
    def scan_red(self):
        if self.frozen_detection and len(self.frozen_detection) == 9:
            ...
            if len(side) == 9 and side[4] == 2:  # FIXED: Check length first
                ...
                self.panel2 = Label(self.root, image=img2, bg="#2b2b2b")
    ...
    def scan_orange(self):
        if self.frozen_detection and len(self.frozen_detection) == 9:
            ...
            if len(side) == 9 and side[4] == 3:  # FIXED: Check length first
                ...
                self.panel3 = Label(self.root, image=img3, bg="#2b2b2b")
    ...
    def scan_blue(self):
        if self.frozen_detection and len(self.frozen_detection) == 9:
            ...
            if len(side) == 9 and side[4] == 4:  # FIXED: Check length first
                ...
                self.panel4 = Label(self.root, image=img4, bg="#2b2b2b")
    ...
    def scan_yellow(self):
        if self.frozen_detection and len(self.frozen_detection) == 9:
            ...
            if len(side) == 9 and side[4] == 5:  # FIXED: Check length first
                ...
                self.panel5 = Label(self.root, image=img5, bg="#2b2b2b")
    ...
    def solve_reset(self):
        ...
        self.face = []
        self.sollution = []
        self.solution_history = []  # NEW: Clear history on reset
        self.frozen_detection = None
    ...
    def step(self):
        self.update_solve()
        if self.sollution:  # NEW: Store move in history before popping
            self.solution_history.append(self.sollution[0])
        try:
    ...
    def go_back(self):  # NEW: Back button functionality
        """Go back one step in the solution"""
        if self.solution_history:
            # Get the last move from history
            last_move = self.solution_history.pop()
            
            # Apply the inverse move to go back
            inverse_moves = {
                "U": "U'", "U'": "U", "U2": "U2",
                "R": "R'", "R'": "R", "R2": "R2", 
                "L": "L'", "L'": "L", "L2": "L2",
            ...
            inverse_move = inverse_moves.get(last_move, last_move)
            
            # Temporarily set the move to the inverse and apply it
            temp_solution = [inverse_move]
            original_solution = self.sollution.copy()
            self.sollution = temp_solution
            self.update_solve()
            
            # Restore the original solution and add the move back
            self.sollution = original_solution
            self.sollution.insert(0, last_move)
            
            # Update the display
            self.update_grid_status()
            print(f"↩️  Went back one step: {last_move}")
        else:
            print("❌ No steps to go back to!")
    ...
    def update_solve(self):
        ...
        if self.solve_status and self.sollution != []:
            ...
            if self.sollution[0] == "U":
                ...
                self.panel = Label(self.root, image=img, bg="#2b2b2b")
    ...
    def solve_cube(self):
        ...
        try:
            ...
            self.solution_history = []  # NEW: Clear history when starting new solution
            self.solve_status = True
            print(f"✅ Solution found: {self.sollution}")
            self.next = Button(self.root, text ="Step",width=15,height=1, command = self.step,bg="#4a4a4a", fg="white", font=("Arial", 10, "bold"), relief="raised", bd=2)
            self.next.place(x=770,y=510,in_=self.root)
            
            # NEW: Add back button when solution is generated
            self.back_btn = Button(self.root, text ="Back",width=15,height=1, command = self.go_back,bg="#4a4a4a", fg="white", font=("Arial", 10, "bold"), relief="raised", bd=2)
            self.back_btn.place(x=620,y=510,in_=self.root)
        except Exception as e:
    ...
    def __init__(self):
        self.root = Tk()
        self.root.title("Rubik's Cube Solver - Dark Theme")
        ...
        self.app = Frame(self.root, bg="#2b2b2b")
        self.app.place(x=738,y=20,in_=self.root)
        self.lmain = Label(self.app, bg="#2b2b2b")
        self.lmain.grid()

        self.zone1 = LabelFrame(self.root,text="Cube Status", bg="#2b2b2b", fg="white", font=("Arial", 13, "bold"))
        self.zone1.place(height=500,width=700,x = 20,y = 20)

        self.zone2 = LabelFrame(self.root,text="Update Status", bg="#2b2b2b", fg="white", font=("Arial", 13, "bold"))
        self.zone2.place(height=70,width=900,x = 10,y = 600)

        self.update_grid_status()

        # Styled buttons with dark theme
        button_style = {"width":15, "height":1, "bg":"#404040", "fg":"white", "font":("Arial", 9, "bold"), "relief":"raised", "bd":2}
        
        scan_green = Button(self.root, text ="Scan Green", command = self.scan_green, **button_style)
        scan_green.place(x=20,y=627,in_=self.root)

        scan_white = Button(self.root, text ="Scan White", command = self.scan_white, **button_style)
        scan_white.place(x=170,y=627,in_=self.root)

        scan_red = Button(self.root, text ="Scan Red", command = self.scan_red, **button_style)
        scan_red.place(x=320,y=627,in_=self.root)

        scan_orange = Button(self.root, text ="Scan Orange", command = self.scan_orange, **button_style)
        scan_orange.place(x=470,y=627,in_=self.root)

        scan_blue = Button(self.root, text ="Scan Blue", command = self.scan_blue, **button_style)
        scan_blue.place(x=620,y=627,in_=self.root)

        scan_yellow = Button(self.root, text ="Scan Yellow", command = self.scan_yellow, **button_style)
        scan_yellow.place(x=770,y=627,in_=self.root)

        solve = Button(self.root, text ="Solve", command = self.solve_cube, width=15,height=1, bg="#00aa00", fg="white", font=("Arial", 10, "bold"), relief="raised", bd=2)
        solve.place(x=770,y=430,in_=self.root)

        reset = Button(self.root, text ="Reset", command = self.solve_reset, width=15,height=1, bg="#aa0000", fg="white", font=("Arial", 10, "bold"), relief="raised", bd=2)
